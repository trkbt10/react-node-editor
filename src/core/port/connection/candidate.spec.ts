...
describe("findNearestConnectablePort", () => {
  ...
  const connectionPoints = new Map<PortKey, Position>([
    [createPortKey("target", "in"), { x: 120, y: 20 }],
    [createPortKey("secondary", "in"), { x: 160, y: 20 }],
  ]);
  ...
  const getConnectionPoint = (nodeId: string, portId: string): Position | null => {
    return connectionPoints.get(createPortKey(nodeId, portId)) ?? null;
  };
