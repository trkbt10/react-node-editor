/**
 * @file Tests for node editor reducer - creating nodes with fixed IDs and connecting them
 */
...
describe("NodeEditor reducer - create node with fixed id and connect", () => {
  it("keeps the provided id via ADD_NODE_WITH_ID and allows connection to that id", () => {
    ...
    const s1 = nodeEditorReducer(defaultNodeEditorData, nodeEditorActions.addNodeWithId(srcNode));
    const s2 = nodeEditorReducer(s1, nodeEditorActions.addNodeWithId(dstNode));
